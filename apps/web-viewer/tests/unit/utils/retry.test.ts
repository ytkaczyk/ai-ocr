...
describe('retry', () => {
  describe('retry', () => {
    ...
    it('should retry up to maxAttempts', async () => {
      ...
      await expect(retry(fn, { maxAttempts: 3 })).rejects.toThrow('Network error');
    ...
    it('should use custom shouldRetry function', async () => {
      ...
      await expect(retry(fn, { shouldRetry })).rejects.toThrow('Custom error');
    ...
    it('should handle non-Error rejections', async () => {
      ...
      await expect(retry(fn, { maxAttempts: 2 })).rejects.toThrow('string error');
    });

    it('should use default maxAttempts of 3', async () => {
      const fn = vi.fn().mockRejectedValue(new Error('Network error'));
      
      await expect(retry(fn)).rejects.toThrow('Network error');
      expect(fn).toHaveBeenCalledTimes(3);
    });
    ...
    it('should stop retrying when shouldRetry returns false', async () => {
      ...
      await expect(retry(fn, { maxAttempts: 5, shouldRetry })).rejects.toThrow('Network error');
      expect(fn).toHaveBeenCalledTimes(2);
    });
  ...
  describe('retryFetch', () => {
    ...
    it('should use custom retry options', async () => {
      ...
      await expect(retryFetch('/api/test', undefined, { maxAttempts: 5, delayMs: 1 })).rejects.toThrow('HTTP 500');
    ...
    it('should throw error after max retries', async () => {
      ...
      await expect(retryFetch('/api/test', undefined, { maxAttempts: 2, delayMs: 1 })).rejects.toThrow('HTTP 500: Internal Server Error');
      expect(fetch).toHaveBeenCalledTimes(2);
    });
